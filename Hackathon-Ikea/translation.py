...
class LangTRans:
    ...
    def datafile(self):
        ...
        if res_lang == config.ISO_LANG_CODE_DE:
            datafile = pickle.load(open(config.EN_GERMAN_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_GR:
            datafile = pickle.load(open(config.EN_GREEK_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_JP:
            datafile = pickle.load(open(config.EN_JAPANESE_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_DA:
            datafile = pickle.load(open(config.EN_DANISH_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_SK:
            datafile = pickle.load(open(config.EN_SLOVAK_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_KR:
            datafile = pickle.load(open(config.EN_KOREAN_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_PT:
            datafile = pickle.load(open(config.EN_PORTUGUESE_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_CZ:
            datafile = pickle.load(open(config.EN_CZECH_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_CN:
            datafile = pickle.load(open(config.EN_CHINESE_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_ET:
            datafile = pickle.load(open(config.EN_ESTONIAN_TRAINING,"rb"))
        elif res_lang == config.ISO_LANG_CODE_ES:
            datafile = pickle.load(open(config.EN_SPANISH_TRAINING,"rb"))
        else:
            datafile = pickle.load(open(config.EN_GERMAN_TRAINING,"rb"))
    ...
    def main_window(self):
        ...
        # this is backup scenarion in case API is not available
        self.window.title("Language Translator")
        self.window.resizable(width=False,height=False)
        self.window.configure(width=520,height=520,bg=BG_COLOR)

        head_label=Label(self.window,bg=BG_COLOR,fg=TEXT_COLOR,text="IKEA MACHINE TRANSLATION",font=FONT_BOLD,pady=10)
        ...
        send_button=Button(bottom_label,text="Send",font=FONT_BOLD,width=8,bg="#fff",command= self.on_enter(None))
...
def result():
    outres = request.get_json()
    if len(outres.keys()) != 3:
        ErrorOut = Response(config.ERROR_BAD_REQUEST,status=400)
        return ErrorOut
    lang_list=[config.ISO_LANG_CODE_GR,config.ISO_LANG_CODE_DE]
    global usr_txt
    global res_lang
    trans={}
    usr_lang = str(outres['ip_lang'])
    usr_txt = str(outres['ip_txt'])
    res_lang = str(outres['trans_lang'])
    print(usr_txt)
    print(res_lang)
    if usr_lang == config.ISO_LANG_CODE_EN:
        if res_lang in lang_list:
            LT = LangTRans()
            print("translatedtext:",trans_txt)
            trans[config.SOURCE_LANG]=usr_lang
            trans[config.MODEL_TRANSLATION] = trans_txt
            trans[config.TRANSLATED_LANG]=res_lang
            return jsonify(trans)
        else :
            ErrorOut = Response(config.ERROR_NOT_FOUND,status=404)
            return ErrorOut
    else:
        ErrorOut = Response(config.ERROR_NOT_FOUND,status=404)
        return ErrorOut
...
def handle_exception(e):
    ...
    fallback_translation(err)
    return err
def fallback_translation(fb_arr):
    translator = deepl.Translator(config.AUTH_KEY)
    fb_trans = translator.translate_text(usr_txt, target_lang=res_lang,formality="more")
    fb_arr[config.TRANSLATED_LANG]=res_lang
    fb_arr[config.FALLBACK_TRANSLATION]=fb_trans.text
    fb_arr[config.SOURCE_LANG]=fb_trans.detected_source_lang
