...
if sys.argv[1] == config.ISO_LANG_CODE_DE:
    TRAINING_FILE = config.EN_GERMAN_TRAINING
    print("TRAINING_FILE",TRAINING_FILE)
    #read dataset file
    with open(config.EN_GERMAN_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')
elif sys.argv[1] == config.ISO_LANG_CODE_GR:
    TRAINING_FILE = config.EN_GREEK_TRAINING
    with open(config.EN_GREEK_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')
elif sys.argv[1] == config.ISO_LANG_CODE_PT:
    TRAINING_FILE = config.EN_PORTUGUESE_TRAINING
    with open(config.EN_PORTUGUESE_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')

elif sys.argv[1] == config.ISO_LANG_CODE_DA:
    TRAINING_FILE = config.EN_DANISH_TRAINING
    with open(config.EN_DANISH_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')

elif sys.argv[1] == config.ISO_LANG_CODE_SK:
    TRAINING_FILE = config.EN_SLOVAK_TRAINING
    with open(config.EN_SLOVAK_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')

elif sys.argv[1] == config.ISO_LANG_CODE_CZ:
    TRAINING_FILE = config.EN_CZECH_TRAINING
    with open(config.EN_CZECH_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')

elif sys.argv[1] == config.ISO_LANG_CODE_CN:
    TRAINING_FILE = config.EN_CHINESE_TRAINING
    with open(config.EN_CHINESE_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')

elif sys.argv[1] == config.ISO_LANG_CODE_ES:
    TRAINING_FILE = config.EN_SPANISH_TRAINING
    with open(config.EN_SPANISH_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')

elif sys.argv[1] == config.ISO_LANG_CODE_ET:
    TRAINING_FILE = config.EN_ESTONIAN_TRAINING
    with open(config.EN_ESTONIAN_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')

elif sys.argv[1] == config.ISO_LANG_CODE_KR:
    TRAINING_FILE = config.EN_KOREAN_TRAINING
    with open(config.EN_KOREAN_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')

elif sys.argv[1] == config.ISO_LANG_CODE_JP:
    TRAINING_FILE = config.EN_JAPANESE_TRAINING
    with open(config.EN_JAPANESE_DICTIONARY,'r',encoding='utf-8') as f:
        rows=f.read().split('\n')
...
pickle.dump({'input_characters':input_characters,'target_characters':target_characters,
             ...
             'num_en_chars':num_en_chars,'num_dec_chars':num_dec_chars},open(TRAINING_FILE,"wb"))
